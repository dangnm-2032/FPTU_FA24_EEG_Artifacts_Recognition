...
print(">>>>>>>>> Filtering 5 label with left filter <<<<<<<<<<<<")
os.makedirs(r'.\pipeline_left', exist_ok=True)
...
print(">>>>>>>>> Normalize 5 label with left filter <<<<<<<<<<<<")
...
for label_ in raw_data_true:
    ...
    dataset_true[label_] = {}
    temp_data, temp_label = create_dataset(data, label, filter_left, scaler)
...
for label_ in raw_data_false:
    ...
    temp_data, temp_label = create_dataset(data, label, filter_left, scaler, epsilon=0.1)
...
data, input_data = get_input(df_eyebrows, filter_left, scaler)
...
data, input_data = get_input(df_right, filter_left, scaler)
y_pred_onehot = get_output(input_data, model)
plot_data_result(
...
data, input_data = get_input(df_left, filter_left, scaler)
y_pred_onehot = get_output(input_data, model)
plot_data_result(
...
data, input_data = get_input(df_both, filter_left, scaler)
y_pred_onehot = get_output(input_data, model)
plot_data_result(
...
data, input_data = get_input(df_teeth, filter_left, scaler)
y_pred_onehot = get_output(input_data, model)
plot_data_result(
